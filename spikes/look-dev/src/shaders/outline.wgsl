...
// downsample. Roberts cross on linear depth, view normals and class ids.

struct Params {
    near: f32,
    far: f32,
    // Tap offsets of the Roberts cross in target pixels (2x SSAA: -1 / +2 = 3 px, about 1.5 px final).
    tap_lo: i32,
    tap_hi: i32,
}
...
fn pair_edge(a: vec2<i32>, b: vec2<i32>) -> f32 {
    ...
    let depth_edge = step(0.015, abs(za - zb) / max(min(za, zb), 1e-4));
    let tn = select(0.6, 0.35, differ || is_solid(ca) || is_solid(cb));
    let normal_edge = step(tn, 1.0 - dot(na.xyz, nb.xyz));
...
fn fs_outline(in: VOut) -> @location(0) vec4<f32> {
    ...
    return vec4<f32>(c.rgb * (1.0 - 0.92 * edge), 1.0);
}
