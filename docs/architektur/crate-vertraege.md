...
| 2026-09-17 | §6 (Skelettanimation: Modul `grimoire_render::figure_clip`, Art `FNP_CLIP`), §9.1, §9.10 (`figure_assets::load_clip`), Formatdoku `figure-clip.md` neu, §2 Regel 10 (Liste der Formatdokumente) | #TBD | A (PO-Freigabe offen) | Spiel (Figuren-Konverter: Clips als `FNP_CLIP` exportieren), Render-A/B | nein (neue Golden-Fixture `figure_clip_v1.bin` samt Herleitung, neue goldene Posen-Hashes) |
