...
| 2026-09-18 | §6 (Rim-Licht der Akteurs-Ebene: `MeshInstance::role`, `MeshRole`, `RimLight` mit `off`/`is_active`/`sanitized`, `StageFrame::rim_light`; wirkt nur auf `MeshRole::Actor`, Vorgabe `Environment`, kein zusätzlicher Pass und kein Platz im Lichtbudget) | #68 | A (PO-Freigabe 2026-09-18, gebündelt mit der Rim-Light-Entscheidung) | Spiel (Prototyp: Figuren als `MeshRole::Actor` melden), Messung und Render (M3) | nein (vier neue Referenzbilder der Szene `actor_rim` für Windows und Linux, keine Erneuerung) |
